...
async def update_task(
    ...
    if not task:
        logger.warning(
            f"Cannot update task {task_id} - not found or not owned by user {user_id}"
        )
        return None
...
async def delete_task(
    ...
    if not task:
        logger.warning(
            f"Cannot delete task {task_id} - not found or not owned by user {user_id}"
        )
        return False
...
async def update_task_status(
    ...
    if not task:
        logger.warning(
            f"Cannot update status of task {task_id} - not found or not owned by user {user_id}"
        )
        return None
    ...
    await session.refresh(task)
    logger.info(
        f"Updated task {task_id} status: {old_status} -> {status} for user {user_id}"
    )
    return task
